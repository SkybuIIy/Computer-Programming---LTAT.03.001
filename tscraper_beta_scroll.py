...
# Maybe for later functionalities
...
def clickbutton():
    if (search_words_entry_2.get() != "" or search_hash_entry_2.get() != "") or (start_date_entry_2.get() != "" and end_date_entry_2.get() != ""):
        ...
        query_button = ttk.Button(scrollable_frame, text="Start Query", command=lambda:[scraper(), scraper_more(), process()])
...
def process():
    ...
    if (search_words_entry_2.get() != "" or search_hash_entry_2.get() != "") or (start_date_entry_2.get() != "" and end_date_entry_2.get() != ""):
        ...
        proceed_button = ttk.Button(scrollable_frame, text="Proceed", command=lambda:[clear(), visualize(listi, hashtags, words, timeframe_start, timeframe_end), visualize(listi2, hashtags2, words2, timeframe_start2, timeframe_end2)]).pack(pady=10)
        return
    else:
        proceed_button = ttk.Button(scrollable_frame, text="Proceed", command=lambda:[clear(), visualize(listi, hashtags, words, timeframe_start, timeframe_end)]).pack(pady=10)
        return

#find all widgets of the window
def all_children():
    _list = scrollable_frame.winfo_children()
...
def visualize(_list, hashtag, query, start, end):
    ...
    if likes == 0:
        content = Label(canvas, text = f'No tweets were found for this query: {", ".join(str(x) for x in query)}. Hashtag(s): {", ".join(str(x) for x in hashtag)}. From {start} to {end}.', bg = "ivory2").pack(padx=10,pady=10)
    elif hashtag[0] != '':
        if query[0] != '':
            content = Label(canvas, text = f'{t_count} tweets were found for the query of {", ".join(str(x) for x in query)} and hashtag(s) {", ".join(str(x) for x in hashtag)} from {start} to {end}.\n\n The most liked tweet was: \n {most_liked_content} \n by {most_liked_content_user} with {likes} likes. \n\n The most retweeted content was: \n {most_retweeted_content} \n by {most_retweeted_content_user} with {retweets} retweets.', bg = "ivory2").pack(padx=10, pady=10)
        else:
            content = Label(canvas, text = f'{t_count} tweets were found for the hashtag(s) {", ".join(str(x) for x in hashtag)} from {start} to {end}.\n\n The most liked tweet was: \n {most_liked_content} \n by {most_liked_content_user} with {likes} likes. \n\n The most retweeted content was: \n {most_retweeted_content} \n by {most_retweeted_content_user} with {retweets} retweets.', bg = "ivory2").pack(padx=10, pady=10)
    elif query[0] != '':
        content = Label(canvas, text = f'{t_count} tweets were found for the query of {", ".join(str(x) for x in query)} from {start} to {end}.\n\n The most liked tweet was: \n {most_liked_content} \n by {most_liked_content_user} with {likes} likes. \n\n The most retweeted content was: \n {most_retweeted_content} \n by {most_retweeted_content_user} with {retweets} retweets.', bg = "ivory2").pack(padx=10, pady=10)
    
    #terminate window and program
    #quit_button = ttk.Button(window, text="Quit", command=window.destroy).grid()
    return
...
window.maxsize(1900, 800)
container = Frame(window, bg="ivory2")
#scrollbar
canvas = Canvas(container, bg="ivory2")
scrollbar = Scrollbar(container, orient='vertical', command=canvas.yview)
scrollable_frame = Frame(canvas, bg="ivory2")
...
l = Label(scrollable_frame, text = "Enter cryptocurrency you want to search for. LEAVE BLANK if specification is not needed.", bg = "ivory2")

# Input parameters
search_hash = Label(scrollable_frame, text="Enter hashtag(s) you want to search for (e.g.: BTC,ETH,DOGE):", bg = "ivory2")
search_hash_entry = Entry(scrollable_frame, width=60, bg = "powder blue", fg="black", borderwidth=5)

search_words = Label(scrollable_frame, text="Enter word(s) you want to search for (e.g.: This,is,an,example):", bg = "ivory2")
search_words_entry = Entry(scrollable_frame, width=60, bg = "powder blue", fg="black", borderwidth=5)

comparison = Label(scrollable_frame, text="If you would like to compare different time periods of the same coin, or different coins, please click below.", bg="ivory2")
comparison_button = ttk.Button(scrollable_frame, text="MORE", command=compare)

query_limit = Label(scrollable_frame, text="Top limit for matching tweets per each day in interval (e.g.: 10):", bg = "ivory2")
query_limit_entry = Entry(scrollable_frame, width=60, bg = "powder blue", fg="black", borderwidth=5)

start_date = Label(scrollable_frame, text="Enter start date of tweets (YYYY-MM-DD):", bg = "ivory2")
start_date_entry = Entry(scrollable_frame, width=60, bg = "powder blue", fg="black", borderwidth=5)

end_date = Label(scrollable_frame, text="Enter end date of tweets (YYYY-MM-DD):", bg = "ivory2")
end_date_entry = Entry(scrollable_frame, width=60, bg = "powder blue", fg="black", borderwidth=5)

user_account = Label(scrollable_frame, text="Specific user account to search tweets from (e.g.: @elonmusk):", bg = "ivory2")
user_account_entry = Entry(scrollable_frame, width=60, bg = "powder blue", fg="black", borderwidth=5)
    
ok_button = ttk.Button(scrollable_frame, text="OK", command=clickbutton)
query_button = ttk.Button(scrollable_frame, text="Start Query", command=lambda:[scraper(), scraper_more(), process()])

# more stuff
label_2 = Label(scrollable_frame, text="Enter 2nd cryptocurrency or time frame you want to analyze (LEAVE BLANK if not needed)", bg = "ivory2")
search_hash_2 = Label(scrollable_frame, text="Enter hashtag(s) you want to search for (e.g.: BTC,ETH,DOGE):", bg = "ivory2")
search_hash_entry_2 = Entry(scrollable_frame, width=60, bg = "powder blue", fg="black", borderwidth=5)

search_words_2 = Label(scrollable_frame, text="Enter word(s) you want to search for (e.g.: This,is,an,example):", bg = "ivory2")
search_words_entry_2 = Entry(scrollable_frame, width=60, bg = "powder blue", fg="black", borderwidth=5)

start_date_2 = Label(scrollable_frame, text="Enter start date of tweets (YYYY-MM-DD):", bg = "ivory2")
start_date_entry_2 = Entry(scrollable_frame, width=60, bg = "powder blue", fg="black", borderwidth=5)

end_date_2 = Label(scrollable_frame, text="Enter end date of tweets (YYYY-MM-DD):", bg = "ivory2")
end_date_entry_2 = Entry(scrollable_frame, width=60, bg = "powder blue", fg="black", borderwidth=5)
